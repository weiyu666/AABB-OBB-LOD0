...
/*
 * parse a subset of texture options, following 解析材质中的纹理
...
//基于ZBrush工具Color调色板显示当前颜色并以提供数值的方法选择颜色，而且还可以选择辅助色，使用描绘工具可以产生混合的色彩效果。
...
bool Model::readOBJ(std::istream& fin, const osgDB::ReaderWriter::Options* options)
{
    ...
    while (fin)
    {
        ...
        else if (strlen(line)>0)
        {
            ...
            //p，l，f：指定多个图元组合成点（point），线（line），或者面（face）
...
/*
对于line，point，face指定的几何元素组合，使用该函数保存元素信息到链表
*/
...
/*
此判断为真的话，翻转几何体法线坐标，以保证OSG与.obj文件的element顺序一致
*/
