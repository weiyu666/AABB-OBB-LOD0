...
// 		retArr->push_back(ptzmax - vHlfDir);
	...
	/*
	协方差矩阵的的元素是实数并且对称,协方差矩阵的特征向量表示OBB包围盒的方向。
	大的特征值对应大的方差，所以应该让OBB包围盒沿着最大特征值对应的特征向量的方向。
	得到特征向量，也就是OBB的坐标轴
	*/
	...
	/*
	静态函数只能在声明它的文件中可见，不能够其他文件所调用，也就是不能被其他的.cpp文件所调用，
	只能被自己的.cpp所调用
	*/
	osg::Matrix saveGetOBBOrientation(osg::ref_ptr<osg::Vec3Array> vertPos, osg::Vec3 &Evals)
	{
		return _getOBBOrientation(vertPos, Evals);
	}

	//获取OBB主轴方向
	static osg::Matrix _getOBBOrientation(osg::ref_ptr<osg::Vec3Array> vertPos, osg::Vec3 &Evals)
	{
		osg::Matrix Cov;
	...
	OBB::OBB(const osg::ref_ptr<osg::Vec3Array>& verts)
	{
		...
		int num = verts->size();//sixe
		osg::Vec3 Evals;
		osg::Matrix matTransform = _getOBBOrientation(verts, Evals);//获取特征向量，就得到obb主轴
