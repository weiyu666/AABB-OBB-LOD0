...
/**
将基本集的所有基本集写到流中，将四边形分解为三角形，将条形线分解为线等
...
void OBJWriterNodeVisitor::apply( osg::Geode &node )
{
    ...
    for ( unsigned int i = 0; i < count; i++ )
    {
        ...
        if ( g != NULL )
        {
            //Save our current stateset and merge with node stateset
